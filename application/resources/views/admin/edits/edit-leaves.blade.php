@extends('layouts.default')
    @section('meta')
    <title>{{ __('admin.Edit Leave of Absence') }} | {{ __('admin.Smart Timesheet') }}</title>
    <meta name="description" content="smart timesheet edit employee leave of absence.">
    @endsection 
    ...
    <div class="container-fluid">
        <div class="row">
        <div class="col-md-12">
            <h2 class="page-title">{{ __('admin.Edit Leave') }}</h2>
        </div>    
        </div>
        ...
        <div class="col-md-12">
            <div class="box box-success">
                ...
                {{ csrf_field() }}
                    ...
                    <div class="field">
                        <p class="ui horizontal divider tiny sub header">{{ __('admin.Manager Privilege') }}</p>
                    </div>
                    <div class="field">
                        <label>{{ __('admin.Status') }}</label>
                        <select class="ui dropdown uppercase" name="status">
                            <option value="Approved" @isset($l->status) @if($l->status == 'Approved') selected @endif @endisset>{{ __('admin.Approved') }}</option>
                            <option value="Pending" @isset($l->status) @if($l->status == 'Pending') selected @endif @endisset>{{ __('admin.Pending') }}</option>
                            <option value="Declined" @isset($l->status) @if($l->status == 'Declined') selected @endif @endisset>{{ __('admin.Declined') }}</option>
                        </select>
                    </div>
                    <div class="field">
                        <label>{{ __('admin.Add Comment') }} <span class="help">({{ __('admin.OPTIONAL') }})</span></label>
                ...
                <div class="box-footer">
                    ...
                    <a href="{{ url('leaves') }}" class="ui black grey small button"><i class="ui times icon"></i> {{ __('admin.Cancel') }}</a>
                </div>
                </form>
