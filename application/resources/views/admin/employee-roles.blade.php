@extends('layouts.default')
    @section('meta')
    <title>{{__('admin.Employee Roles')}} | {{__('admin.Smart Timesheet')}}</title>
    ...
    <div class="container-fluid">
        <div class="row">
            <h2 class="page-title">{{__('admin.User Roles')}}
                <button class="ui positive button mini offsettop5 btn-add float-right"><i class="ui icon plus"></i>{{__('admin.Add')}}</button>
                <a href="{{ url('users') }}" class="ui basic blue button mini offsettop5 float-right"><i class="ui icon chevron left"></i>{{__('admin.Return')}}</a>
            </h2>
        </div>
