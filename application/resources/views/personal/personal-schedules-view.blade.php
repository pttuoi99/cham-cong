@extends('layouts.personal')
    @section('meta')
    <title>My Schedules | Smart Timesheet</title>
    <meta name="description" content="smart timesheet my schedules, view my schedule records, view present and previous schedules.">
    @endsection
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
            <h2 class="page-title">{{__('users.MY SCHEDULES')}}</h2>
            </div>    
        </div>
            ...
            <div class="box box-success">
                <div class="box-body reportstable">
                    <form action="" method="get" accept-charset="utf-8" class="ui small form form-filter" id="filterform">
                        ...
                        <div class="inline two fields">
                            <div class="three wide field">
                                <label>{{__('users.Date Range')}}</label>
                            ...
                            <button id="btnfilter" class="ui button positive small"><i class="ui icon filter alternate"></i>{{__('users.Filter')}}</button>
                        </div>
                    </form>
                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example" data-order='[[ 6, "desc" ]]'>
                        <thead>
                            <tr>
                                <th>{{__('users.Start Time')}}</th>
                                <th>{{__('users.Off Time')}}</th>
                                <th>{{__('users.Total Hours')}}</th>
                                <th>{{__('users.Rest Day')}}<span class="help">(s)</span></th>
                                <th>{{__('users.From')}}<span class="help">{{__('users.(Date)')}}</span></th>
                                <th>{{__('users.To')}}<span class="help">{{__('users.(Date)')}}</span></th>
