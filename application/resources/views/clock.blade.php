@extends('layouts.clock')
        ...
        <div class="fixedcenter">
            <div class="clockwrapper">
                <div class="field mt-4">
                    <select id="schedule"  class="ui search dropdown getid uppercase" name="">
                        {{-- <option>-- Chọn Lịch Trình --</option> --}}
                        @if(count($schedules) > 0)
                        @foreach($schedules as $item)
                            <option value="{{$item->id}}">{{$item->intime}} - {{$item->outime}}  | {{$item->datefrom}} - {{$item->dateto}}</option>
                        @endforeach
                        @else
                        <option value="0">chưa có lịch trình</option>
                       @endif
                    </select>
                </div>
                
                <div class="clockinout">
                    <button class="btnclock timein active" data-type="timein">{{__('users.Time In')}}</button>
                    <button class="btnclock timeout" data-type="timeout">{{__('users.Time Out')}}</button>
                </div>
            </div>
                ...
                <div class="userinput">
                    ...
                    <form action="" method="get" accept-charset="utf-8" class="ui form">
                        ...
                        <div class="inline field">
                            ...
                            <button id="btnclockin" type="button" class="ui positive large icon button">{{__('users.Confirm')}}</button>
    ...
    $('#btnclockin').click(function(event) {
        var url, type, comment, schedule_id;
        url = $("#_url").val();
        ...
        $.ajax({
            ...
            type: 'get',
            dataType: 'json',
            data: { type: type, clock_comment: comment, schedule_id: schedule_id},
