...
class ReportsController extends Controller
{
	...
	public function getEmpAtten(Request $request) {
		...
		if ($id == null AND $datefrom == null AND $dateto == null) {
			$data = table::attendance()->select('idno', 'date', 'employee', 'timein', 'timeout', 'totalhours', 'minutes_late', 'minutes_early')->get();
			return response()->json(["data" => $data]);
		}

		if($id !== null AND $datefrom == null AND $dateto == null ) {
			 $data = table::attendance()->where('idno', $id)->select('idno', 'date', 'employee', 'timein', 'timeout', 'totalhours', 'minutes_late', 'minutes_early')->get();
			 $totalminutes_late = table::attendance()->where('idno', $id)->sum('minutes_late');
			 $totalminutes_early = table::attendance()->where('idno', $id)->sum('minutes_early');
			 $totaltimes_late = table::attendance()->where('idno', $id)->where('status_timein', 'Late Arrival')->count('status_timein');
			 $totaltimes_early = table::attendance()->where('idno', $id)->where('status_timeout', 'Early Departure')->count('status_timeout');
			 $totaltimes_notimeout = table::attendance()->where('idno', $id)->where('timeout', 'no timeout')->count('timeout');
			
			return response()->json([
				"data" => $data,
				"totaltimes_late" => $totaltimes_late,
				"totaltimes_early" => $totaltimes_early,
				"totalminutes_late" => $totalminutes_late,
				"totalminutes_early" => $totalminutes_early,
				"totaltimes_notimeout" => $totaltimes_notimeout,
				]);

		} elseif ($id !== null AND $datefrom !== null AND $dateto !== null) {
			$data = table::attendance()->where('idno', $id)->whereBetween('date', [$datefrom, $dateto])->select('idno', 'date', 'employee', 'timein', 'timeout', 'totalhours', 'minutes_late', 'minutes_early')->get();
			$totalminutes_late = table::attendance()->where('idno', $id)->whereBetween('date', [$datefrom, $dateto])->sum('minutes_late');
			$totalminutes_early = table::attendance()->where('idno', $id)->whereBetween('date', [$datefrom, $dateto])->sum('minutes_early');
			$totaltimes_late = table::attendance()->where('idno', $id)->whereBetween('date', [$datefrom, $dateto])->where('status_timein', 'Late Arrival')->count('status_timein');
			$totaltimes_early = table::attendance()->where('idno', $id)->whereBetween('date', [$datefrom, $dateto])->where('status_timeout', 'Early Departure')->count('status_timeout');
			$totaltimes_notimeout = table::attendance()->where('idno', $id)->whereBetween('date', [$datefrom, $dateto])->where('timeout', 'no timeout')->count('timeout');
			...
			$data = table::attendance()->whereBetween('date', [$datefrom, $dateto])->select('idno', 'date', 'employee', 'timein', 'timeout', 'totalhours', 'minutes_late', 'minutes_early')->get();
			return response()->json(["data" => $data]);
		} 
	}
